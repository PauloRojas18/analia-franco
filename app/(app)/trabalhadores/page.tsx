// app/(app)/trabalhadores/page.tsx
"use client"
...
export default function TrabalhadoresPage() {
  ...
  const [editando, setEditando] = useState<Trabalhador | null>(null)
  ...
  const buscarTrabalhadores = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch(`/api/trabalhadores?search=${encodeURIComponent(search)}`)
  ...
  useEffect(() => {
    const t = setTimeout(buscarTrabalhadores, 300)
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        // Trabalhadores podem vincular com qualquer categoria
        const res = await fetch(`/api/vinculos?nome=${encodeURIComponent(buscaVinculo)}&excluirTipo=trabalhador`)
        const data = await res.json()
  ...
  function abrirEditar(t: Trabalhador) {
    setEditando(t)
    setForm({ nome: t.nome, telefone: t.telefone, email: t.email })
    setErroForm(""); setVinculos([]); setBuscaVinculo(""); setModalAberto(true)
    buscarVinculos(t.codigoBarras)
  }
  ...
  async function adicionarVinculo(resultado: ResultadoBusca) {
    ...
    try {
      await fetch("/api/vinculos", {
        ...
        body: JSON.stringify({
          codigoBarras: editando.codigoBarras, tipoPrincipal: "trabalhador",
  ...
  async function salvar() {
    ...
    try {
      const res = await fetch(
        editando ? `/api/trabalhadores/${editando.id}` : "/api/trabalhadores",
      ...
      if (!editando && vinculos.length > 0) {
        await Promise.all(vinculos.map(v =>
          fetch("/api/vinculos", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ codigoBarras: salvo.codigoBarras, tipoPrincipal: "trabalhador", codigoSecund: v.codigoBarras, tipoSecund: v.tipo }),
          })
        ))
      }
      fecharModal(); buscarTrabalhadores()
  ...
  async function deletar(id: number) {
    try {
      await fetch(`/api/trabalhadores/${id}`, { method: "DELETE" })
      setConfirmandoId(null); buscarTrabalhadores()
                ...
                <tbody>
                  {trabalhadores.map((t) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">{t.telefone}</td>
                      <td className="px-4 py-3 hidden lg:table-cell text-muted-foreground">{t.email}</td>
