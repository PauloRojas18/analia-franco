...
export default function PacientesPage() {
  ...
  const [abaModal, setAbaModal] = useState<"cadastro" | "primeira-vez">("cadastro")
  ...
  function abrirNovo() {
    ...
    setVinculos([]); setBuscaVinculo(""); setPrimeiraVez(""); setAbaModal("cadastro"); setModalAberto(true)
  }
  ...
  function fecharModal() {
    ...
    setPrimeiraVez(""); setAbaModal("cadastro")
  }
  ...
  async function salvar() {
    ...
    try {
      ...
      if (!editando && primeiraVez.trim()) {
        await fetch(`/api/pacientes/${salvo.id}/observacoes`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ numero: 1, descricao: primeiraVez.trim() }),
        })
      }
  ...
  const abas = [
    ...
    ...(!editando ? [{ key: "primeira-vez" as const, label: "1ª Vez" }] : []),
  ]
                    ...
                    <tr key={p.id} className="border-b border-border last:border-0 transition-colors"
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <Button variant="default" size="icon" title="Prontuário" style={{cursor:"pointer"}}
                          ...
                          <Button variant="outline" size="icon" style={{cursor:"pointer"}} onClick={() => abrirEditar(p)}>
          ...
          <div className="relative z-10 w-full max-w-md flex flex-col" style={{
            ...
            <div className="px-8 pt-7 pb-0 shrink-0">
              ...
              <div className="flex" style={{ borderBottom: "1px solid var(--border)" }}>
                {abas.map((aba) => (
                  <button key={aba.key}
                    onClick={() => setAbaModal(aba.key)}
                    style={{
                      padding: "10px 16px", fontSize: 14, fontWeight: 500,
                      background: "none", border: "none", cursor: "pointer",
                      borderBottom: abaModal === aba.key ? "2px solid var(--primary)" : "2px solid transparent",
                      color: abaModal === aba.key ? "var(--primary)" : "var(--muted-foreground)",
                      marginBottom: -1, transition: "color 0.15s",
                      display: "flex", alignItems: "center", gap: 6,
                    }}>
                    {aba.label}
                    {aba.key === "primeira-vez" && primeiraVez.trim() && (
                      <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "#1e6b94" }} />
                    )}
                  </button>
                ))}
              </div>
            ...
            {/* Conteúdo rolável */}
            <div className="p-8 pt-6 overflow-y-auto modal-scroll flex-1">

              {/* Aba: Cadastro (inclui vínculos) */}
              {abaModal === "cadastro" && (
              ...
              {/* Aba: 1ª Vez — apenas no cadastro novo */}
              {abaModal === "primeira-vez" && !editando && (
                <div className="flex flex-col gap-3">
                  <div>
                    <p className="text-sm font-medium mb-1" style={{ color: "var(--foreground)" }}>
                      Observações da primeira visita
                    </p>
                    <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>
                      Registre as observações do primeiro atendimento. Pode ser editado depois no prontuário.
                    </p>
                  </div>
                  <textarea
                    placeholder="Descreva as observações da primeira visita / consulta inicial..."
                    value={primeiraVez}
                    onChange={e => setPrimeiraVez(e.target.value)}
                    rows={9}
                    className="w-full resize-none text-sm outline-none p-4 leading-relaxed"
                    style={{
                  ...
                  <p className="text-xs text-right" style={{ color: "var(--muted-foreground)" }}>
                    {primeiraVez.length} caracteres{!primeiraVez.trim()}
                  </p>
                </div>
              )}

              {erroForm && (
                <p className="mt-4 text-sm" style={{ color: "var(--destructive)" }}>{erroForm}</p>
              )}
              ...
              <div className="mt-6 flex gap-2">
                ...
                <button onClick={salvar} disabled={salvando}
                  ...
                  {salvando && (
                    <div style={{ width: 15, height: 15, border: "2px solid rgba(255,255,255,0.4)", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.8s linear infinite", flexShrink: 0 }} />
                  )}
