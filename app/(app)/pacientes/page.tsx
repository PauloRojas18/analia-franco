...
const FORM_VAZIO: FormData = { nome: "", especialidade: "", telefone: "", endereco: "" }
...
export default function PacientesPage() {
  ...
  const buscarPacientes = useCallback(async () => {
    ...
    try {
      ...
      setPacientes(data)
    } catch { console.error("Erro ao buscar pacientes") }
    finally { setLoading(false) }
  }, [search])

  useEffect(() => {
    const t = setTimeout(buscarPacientes, 300)
    return () => clearTimeout(t)
  }, [buscarPacientes])

  // Busca vínculos ao editar
  const buscarVinculos = useCallback(async (codigoBarras: string) => {
    try {
      const res = await fetch(`/api/vinculos?nome=&codigoBarras=${codigoBarras}`)
      // Busca os vínculos existentes para este paciente
      const resVinculos = await fetch(`/api/vinculos/lista?codigoBarras=${codigoBarras}`)
      if (resVinculos.ok) {
        const data = await resVinculos.json()
        setVinculos(data)
      }
    } catch { console.error("Erro ao buscar vínculos") }
  }, [])

  // Busca por nome para vincular
  useEffect(() => {
  ...
  function abrirEditar(p: Paciente) {
    setEditando(p)
    setForm({ nome: p.nome, especialidade: p.especialidade, telefone: p.telefone, endereco: p.endereco })
    setErroForm("")
    setVinculos([])
    setBuscaVinculo("")
    setModalAberto(true)
  ...
  async function adicionarVinculo(resultado: ResultadoBusca) {
    if (!editando) {
      // Se ainda está criando, guarda localmente para salvar depois
  ...
  async function salvar() {
    if (!form.nome || !form.especialidade || !form.telefone || !form.endereco) {
    ...
    try {
      const res = await fetch(
        editando ? `/api/pacientes/${editando.id}` : "/api/pacientes",
        {
      ...
      // Salva vínculos pendentes (apenas ao criar)
      if (!editando && vinculos.length > 0) {
        await Promise.all(vinculos.map(v =>
          fetch("/api/vinculos", {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              codigoBarras: pacienteSalvo.codigoBarras,
              tipoPrincipal: "paciente",
  ...
  const campos = [
    ...
    { key: "especialidade", label: "Especialidade", placeholder: "Ex: Fisioterapia",    icon: "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" },
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground hidden md:table-cell">Especialidade</th>
                ...
                <tbody>
                  {pacientes.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 hidden md:table-cell text-muted-foreground">{p.especialidade}</td>
            ...
            <div className="p-8">
              ...
              <div className="mt-5 flex flex-col gap-3">
                  ...
                  {mostrarDropdown && resultadosBusca.length > 0 && (
                    <div ref={dropdownRef}
                      ...
                      {resultadosBusca.map((r) => (
                        <button key={`${r.tipo}-${r.codigoBarras}`}
                ...
                <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>
                  Pessoas vinculadas usam o mesmo crachá. Ao ler, o sistema pergunta qual presença registrar.
                </p>
              </div>
              ...
              {!editando && (
                <p className="mt-3 text-xs" style={{ color: "var(--muted-foreground)" }}>
                  O código de crachá será gerado automaticamente com prefixo <span className="font-mono font-semibold">CEFASP</span>.
                </p>
              )}
